<div class="page-content x_panel">
	<div class="form-container">
		<div class="row" style="padding: 0 16px;">
			<div class="col-md-12">
				<h5><i class="fa fa-list-alt"></i> Lengkapi form ini untuk menambah data penjualan</h5>
				<?php 
                    ...
                    $carikode = $this->db->query("SELECT MAX(no_penjualan) FROM tbl_penjualan WHERE tgl_penjualan = '$tgl_penjualan'")->result_array();
					foreach ($carikode[0] as $key => $value) {
						if($value) {
                        $nilaikode = substr($value, 13);
                        $kode = (int) $nilaikode;
                        $kode = $kode + 1;
				...
				<form method="post" id="form_penjualan" autocomplete="off">
					<div class="position-relative row form-group">
						<div class="col-sm-4">
							<input name="no_penjualan" id="no_penjualan" placeholder="nomor penjualan" type="hidden"
								class="form-control form-control-sm" value="<?php echo $no_penjualan; ?>">
						</div>
						<!-- <label for="tanggal_pjl" class="col-sm-2 col-form-label">Periode Penjualan</label> -->
						<div class="col-sm-4">
							<input name="tanggal_pjl" id="tanggal_pjl" type="hidden"
					...
					<div class="row kotak-form-tabel-penjualan">
						<div class="col-md-3 kotak-form-obat-terjual" style="display: ;">
							...
							<div class="position-relative form-group">
								...
								<div class="input-group">
									<input type="number" class="form-control" id="jml_obat" name="jml_obat"
										aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
									<span class="input-group-btn">
										<button type="button" id="span_satuan" class="btn btn-primary">Satuan</button>
									</span>
								</div>
							</div>
							<div class="position-relative form-group">
								...
								<div class="input-group">
									...
									<input type="number" class="form-control" name="toth_obat" id="toth_obat"
										aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
								</div>
							</div>
							<div class="position-relative form-group text-right mt-2 mb-2">
								...
								<button type="button" class="btn btn-info btn-sm" id="tambah_obat"><i class="fa fa-plus-square"></i> T ambah</button>
							</div>
							<!-- </form> -->
						</div>
						<div class="col-md-3 kotak-form-pembayaran" style="display:none;">
							...
							<div class="position-relative form-group">
								...
								<div class="text-right">
									<h3 class="text-muted well well-sm no-shadow">Rp. <span id="total_pembayaran">127500</span></h3>
								</div>
							</div>
							<div class="position-relative form-group">
								...
								<div class="input-group">
									...
									<input name="jml_uang" id="jml_uang" placeholder="" type="number"
							...
							<div class="position-relative form-group">
								...
								<div class="input-group">
									...
									<input type="text" class="form-control" id="jml_kembali" name="jml_kembali"
										aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm"
										readonly="">
								</div>
							</div>
						...
						<div class="col-md-9 kotak-tabel-obat-terjual">
							<table class="table table-striped">
								<thead class="thead-dark">
									<tr>
										<th class="text-left row">Kode</th>
								...
								<tfoot>
									<tr id="baris_kosong">
										<td colspan="8" class="text-center text-danger">Belum ada data penjualan</td>
									</tr>
									<tr class="baris_total" style="display: none;">
										<td colspan="7" class="text-right" style="font-weight: bold;">Total : <span
												id="total_penjualan"></span><input type="hidden"
												name="hidden_totalpenjualan" id="hidden_totalpenjualan"></td>
										<td class="td-opsi">
							...
							<div class="baris_total text-right" style="display: none;">
								<button type="button" name="lanjut_pembayaran" id="lanjut_pembayaran"
									class="btn btn-primary btn-sm" style="font-size: 12px;">Lanjut pembayaran</button>
								<button type="button" name="lanjut_pembayaran" id="tambah_obat_lagi"
									class="btn btn-primary btn-sm" style="font-size: 12px; display: none;">Tambah obat
									lagi</button>
							</div>
						</div>
					</div>
					<div class="text-right tombol-kanan">
						<input type="submit"  name="simpan_penjualan" id="simpan_penjualan" class="btn btn-info"
							value="Simpan">
					</div>
				</form>
			</div>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h1 class="modal-title" id="exampleModalLabel">Data Obat</h1>
			...
			<div class="modal-body">
				<table id="datatable" class="table table-striped table-bordered">
					...
					<tbody>
						<?php 
            $query_tampil = $this->db->query("SELECT A.*, B.name AS category_name, C.name AS unit_name, D.name AS
                                    bentuk_name, E.* FROM dataobat A
									...
									INNER JOIN stokexpobat E ON A.kd_obat = E.kd_obat WHERE E.tgl_exp > date_add(CURDATE(), INTERVAL 10 DAY) AND E.stok > 0 ORDER BY A.nm_obat ASC")->result_array();
			foreach ($query_tampil as $key => $data) {
        ?>
						<tr>
							...
							<td class="td-opsi">
								<button class="btn-transition btn btn-outline-dark btn-sm" title="pilih"
									...
									data-satuan="<?php echo $data['unit_name']; ?>"
									data-stok="<?php echo $data['stok']; ?>" data-exp="<?php echo $data['tgl_exp']; ?>">
									pilih
								</button>
							</td>
						</tr>
	...
	$(document).ready(function () {
		...
		$("#tambah_obat").click(function () {
			...
			if (kode == "") {
				...
				Swal.fire(
					'Stok tidak cukup',
					'maaf, jumlah ' + jumlah + ' stok tidak mencukupi. stok yang tersedia sebanyak ' +
					stok + ' ' + satuan,
					'warning'
				)
			} else {
				...
				output += '<td>' + kode + ' <input type="hidden" name="hidden_kdobat[]" id="td_kd_obat' +
				...
				output += '<td class="text-right">' + harga +
					' <input type="hidden" name="hidden_hrgobat[]" id="td_hrgobat' + count +
					'" class="td_hrgobat" value="' + harga + '"></td>';
				output += '<td class="text-center">' + jumlah +
					' <input type="hidden" name="hidden_jmlobat[]" id="td_jmlobat' + count +
					'" class="td_jmlobat" value="' + jumlah + '"></td>';
				output += '<td class="text-center">' + satuan +
					' <input type="hidden" name="hidden_satobat[]" id="td_satobat' + count +
					'" class="td_satobat" value="' + satuan + '"></td>';
				output += '<td class="text-right">' + subtotal +
					' <input type="hidden" name="hidden_subtotal[]" id="td_subtotal' + count +
					'" class="td_subtotal" value="' + subtotal + '"></td>';
				output +=
				...
				Swal.fire({
					...
					if (simpan.value) {
						...
						if (count_data > 0) {
							...
							$.ajax({
								url: "<?php echo site_url('penjualan/savepenjualan'); ?>",
								method: "POST",
								data: form_data,
								success: function (data) {
									Swal.fire({
										...
										if (ok.value) {
											window.open(
													"<?= site_url('penjualan/laporan_penjualan')?>/"+no_penjualan);
											location.reload();
										}
									})
								}
